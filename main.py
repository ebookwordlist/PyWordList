...
def process_chapter(sentences, my_words_dict):
    wordlist = reduce(lambda x, y: x + y,
                      [nltk.word_tokenize(s) for s in sentences])
    stop_words = set(stopwords.words("english") +
                     split(string.punctuation) + ["“", "”"])
    filtered_list = [word for word in wordlist if word.casefold()
                     not in stop_words]
    filtered_list = remove_duplicate_keep_order(filtered_list)
    filtered_list = [
        word for word in filtered_list if word.casefold() not in my_words_dict]
    filtered_list = [
        word for word in filtered_list if word.casefold() in frequency_list]
...
def get_default_export_path():
    export_path = path.join(path.expanduser(
        "~"), 'Desktop/english_world_list.xlsx')
    return export_path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exportPath", help="the path of export xlsx",
                        type=str, default=get_default_export_path())
    parser.add_argument(
        "--level", help="your english world level/count default is 3000", type=int, default=3000)
    args = parser.parse_args()
    print(args.importPath)
    print(args.exportPath)
    print(args.level)
    export_ebook_wordlist(book_path=args.importPath,
                          export_path=args.exportPath, my_word_count=args.level)
